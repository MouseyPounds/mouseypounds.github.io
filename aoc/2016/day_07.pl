...
(my ($tls, $ssl)) = count_tls_and_ssl(\@list);
print "P1: The number of addresses that support TLS is $tls.\n";
print "P2: The number of addresses that support TLS is $ssl.\n";

sub count_tls_and_ssl {
	my $ip = shift;
	
	my $tls = 0;
	my $ssl = 0;
	for (my $i = 0; $i <= $#$ip; $i++) {
		my @pieces = split /(\[[^\[\]]+\])/, $ip->[$i];
		my $in_abba = 0;
		my $out_abba = 0;
		my @in_aba = ();
		my @out_aba  = ();
		foreach my $p (@pieces) {
			if ($p =~ /(\w)(?!\g1)(\w)\g2\g1/) { ($p =~ /^\[/) ? $in_abba++ : $out_abba++; } 
			my @aba = $p =~ /(?=((?<c1>\w)(?!\g{c1})\w\g{c1}))/g;
			if (scalar(@aba)) { if ($p =~ /^\[/) { push @in_aba, @aba; } else { push @out_aba, @aba; } }
		}
		my $ssl_match = 0;
		foreach my $aba (@out_aba) { 
			next if (length $aba != 3);
			my $a = substr $aba, 0, 1;
			my $b = substr $aba, 1, 1;
			$ssl_match = 1 if (any {$_ eq "$b$a$b"} @in_aba);
		}
		$ssl++ if ($ssl_match);
		$tls++ if ($out_abba and not $in_abba);
	}
