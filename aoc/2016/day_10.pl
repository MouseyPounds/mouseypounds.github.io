...
my $bot = get_bot(\@list, 61, 17);
print "P1: The bot who must compare value 61 and value 17 microchips is $bot\n";
...
sub get_bot {
	...
	# We need to operate on a copy because we will be altering the command list
	my @cmd = @$ref;
	
	my %factory = ( 'bot' => {}, 'output' => {}, 'mc1' => $mc1, 'mc2' => $mc2 );
	for (my $i = 0; $i <= $#cmd; $i++) {
		if ($cmd[$i] =~ /^value (\d+) goes to (\w+) (\d+)/) {
			$factory{$2}{$3} = [] unless (defined $factory{$2}{$3});
			give_chip(\%factory, $2, $3, $1);
		} elsif ($cmd[$i] =~ /^(\w+) (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/) {
			(my ($low, $high)) = take_chips(\%factory, $1, $2);
			if (defined $low and defined $high) {
				give_chip(\%factory, $3, $4, $low);
				give_chip(\%factory, $5, $6, $high);
			} else {
				# Can't take these chips yet, so delay command.
				push @cmd, $cmd[$i];
			}
		}
	}
	print "P2: Product of values in outputs 0, 1, and 2 is " . 
		$factory{'output'}{0}[0] * $factory{'output'}{1}[0] * $factory{'output'}{2}[0] . "\n";
	
	return $factory{'wanted_bot'};
}
...
sub give_chip {
	...
	$facref->{$type}{$id} = [] unless (defined $facref->{$type}{$id});
	push @{$facref->{$type}{$id}}, $chip;
	warn "Bot $id overloaded" if ($type eq 'bot' and scalar @{$facref->{$type}{$id}} > 2);
	if ( (any { $_ == $facref->{'mc1'} } @{$facref->{$type}{$id}}) and (any { $_ == $facref->{'mc2'} } @{$facref->{$type}{$id}}) ) {
		$facref->{'wanted_bot'} = $id;
	}
}
...
sub take_chips {
	...
	if (not exists $facref->{$type}{$id} or scalar @{$facref->{$type}{$id}} < 2) {
		return undef, undef;
	} else {
		my @chips = splice @{$facref->{$type}{$id}}, 0, 2;
		return min(@chips), max(@chips);
	}
}
