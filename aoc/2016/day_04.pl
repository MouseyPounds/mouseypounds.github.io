...
(my ($count, $room)) = get_sum();
print "P1: Sum of sector IDs for valid rooms is $count\n";
print "P2: Sector ID for North Pole Object Storage is $room\n";


sub get_sum {
	#my $part2 = shift; $part2 = 0 unless (defined $part2);
	my $sum = 0;
	my $npos = "";

	for (my $i = 0; $i <= $#list; $i++) {
		(my ($name, $sector, $checksum)) = $list[$i] =~ /^([-\w]+)\-(\d+)\[(\w+)\]/;
		(my @letters) = $name =~ /([a-z])/g;
		my %count = ();
		foreach my $char (@letters) { $count{$char}++ };
		my $checkstr = join('', (sort { $count{$b} <=> $count{$a} or $a cmp $b } keys %count));
		my $valid = ($checksum eq substr $checkstr, 0, 5);
		if ($valid) {
			$sum += $sector;
			$name = decode_name($name, $sector);
			$npos = $sector if ($name eq 'northpole object storage');
		}
	}
...
sub decode_name {
	...
	for (my $c = 0; $c <= $#chars; $c++) {
		next if ($chars[$c] eq ' ');
		#print "  Decoding $chars[$c]...\n";
		#print "    ord c = " . ord($chars[$c]) . ", ord a = " . ord('a') . 
		$chars[$c] = chr( $base + ((ord($chars[$c]) - $base + $sec) % 26) );
	}
	return join('', @chars);
}
