...
my $net_ref = analyze_network(\@lines);
print "\nP1 solution: There are $net_ref->{'pairs'} viable pairs of nodes.\n";

#print_nodes($net_ref);

my $min_path = move_data($net_ref);
print "\nP2 solution: It will take $min_path moves to move the data to the access point.\n";

# Part 2 is difficult to solve by traditional methods. However there are several important features of the network:
# - The only viable moves are between an empty node (there is one at start) and a non-empty node, like a tile-slide puzzle.
# - There are several "block" nodes which have such high usage (400+) and low space (<20) that they never exchange data.
#   The block nodes seem to be always clustered together in a kind of wall, which has some gaps to get through.
# - Once the data has moved a tile (putting the empty node behind it,) moving it a second time is a 5-move process:
#   .D_   .D.  .D.  .D.  _D.  D_.
#   ...   .._  ._.  _..  ...  ...
# This leads us to the following approach:
# Count how many moves it takes to get the empty node to the Data location (avoiding the wall), then take that value
# and add 5 * the number of moves to get from there to the access point. Wall is assumed to be at y=2 or further south.
...
sub move_data {
	...
	my $ey = 0;
	my %wall = ();
	# Find the empty node and walls
	for (my $y = 0; $y <= $net_ref->{'max_y'}; $y++) {
		for (my $x = 0; $x <= $net_ref->{'max_x'}; $x++) {
			if ($net_ref->{'node'}{"$x,$y"}{'used'} == 0) {
				$ex = $x; $ey = $y;
			} elsif ($net_ref->{'node'}{"$x,$y"}{'used'} > 400) {
				$wall{"$x,$y"} = '';
			}
		}
	}
	# Get past the wall and to the top row. Go right if we can, otherwise left.
	while ($ey > 0) {
		my $above = $ey - 1;
		if (exists $wall{"$ex,$above"}) {
			my $can_go_right = 0;
			for (my $x = $ex + 1; $x <= $net_ref->{'max_x'}; $x++) {
				if (not exists $wall{"$x,$above"}) {
					$dist += $x - $ex;
					$ex = $x;
					$can_go_right = 1;
					last;
	...
	# Move to data location and then do the 5-tile cycle back to the access point.
	$dist += $net_ref->{'max_x'} - $ex;
	$dist += 5 * ($net_ref->{'max_x'} - 1);
...
sub analyze_network {
	...
	my %network = ( 'max_x' => 0, 'max_y' => 0, 'pairs' => 0, 'node' => {}, );
	
	for (my $i = 0; $i <= $#$out; $i++) {
		if ((my ($x, $y, $size, $used, $avail)) = $out->[$i] =~ m/(\d+)/g) {
			$network{'node'}{"$x,$y"} = { 'used' => $used, 'avail' => $avail, };
			$network{'max_x'} = $x if ($x > $network{'max_x'});
			$network{'max_y'} = $y if ($y > $network{'max_y'});
		}
	}
	print "Read disk usage for " . scalar(keys %{$network{'node'}}) . " nodes.\n";

	my $viable_pairs = 0;
	foreach my $a (keys %{$network{'node'}}) {
		foreach my $b (keys %{$network{'node'}}) {
			next if ($a eq $b);
			if ($network{'node'}{$a}{'used'} > 0 and $network{'node'}{$b}{'avail'} >= $network{'node'}{$a}{'used'}) {
				$network{'pairs'}++;
				warn "Pair involves a non-empty node ($b)\n" unless ($network{'node'}{$b}{'used'} == 0);
			}
		}
	}
	return \%network;
...
sub print_nodes {
	...
	print "Network Node Map\nA = Access Point, D = Data, # = Full Node, _ = Empty Node\n\n";
	...
	for (my $y = 0; $y <= $net_ref->{'max_y'}; $y++) {
		...
		for (my $x = 0; $x <= $net_ref->{'max_x'}; $x++) {
			...
			if ($x == 0 and $y == 0) {
				$char = 'A';
			} elsif ($x == $net_ref->{'max_x'} and $y == 0) {
				$char = 'D';
			} elsif ($net_ref->{'node'}{"$x,$y"}{'used'} > 400) {
				$char = '#';
			} elsif ($net_ref->{'node'}{"$x,$y"}{'used'} == 0) {
				$char = '_';
			}
			$line .= $char;
		}
