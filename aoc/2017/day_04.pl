...
my $total_valid = 0;
my $total_valid_p2 = 0;
foreach my $p (@passphrase) {
	my %uniq = ();
	my %sorted = ();
	my $valid = 1;
	my $anagram = 0;

	foreach my $w (split(' ', $p)) {
		if (exists $uniq{$w}) {
			$valid = 0;
		} else {
			$uniq{$w} = '';
	...
	$total_valid_p2++ if ($valid and not $anagram);
	$total_valid++ if $valid;
}
print "P1: Total valid passphrases banning duplicate words: $total_valid\n";
print "P2: Total valid passphrases banning duplicate words and anagrams: $total_valid_p2\n";
