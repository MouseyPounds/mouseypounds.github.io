...
my %tower = ();
my $root = parse_tower(\%tower, \@lines);
print "P1: The root of the tower is $root.\n";

my $weight = balance_tower(\%tower, $root);
print "P2: The weight needed to balance the tower is $weight.\n";

sub parse_tower {
	my $towerref = shift;
	my $inref = shift;
	
	foreach my $line (@$inref) {
		(my ($prog, $weight, $children)) = $line =~ /^(\w+) \((\d+)\)\W*(\w?.*)$/;
		$towerref->{$prog}{'weight'} = $weight;
		if (defined $children) {
			my @children = split(', ', $children);
			$towerref->{$prog}{'child'} = [@children];
			foreach my $c (@children) { $towerref->{$c}{'parent'} = $prog; }
		}
	}
	
	my $root = (keys %$towerref)[0];
	while (exists $towerref->{$root}{'parent'}) { $root = $towerref->{$root}{'parent'}; }
	return $root;
}
...
sub balance_tower {
	...
	my %count = ();
	my $problem = "";
	if (exists $towerref->{$root}{'child'}) {
		foreach my $c (@{$towerref->{$root}{'child'}}) {
			my $w = get_weight($towerref, $c);
			$count{$w} = 0 unless exists $count{$w};
			$count{$w}++;
			$wt{$w} = $c;
		}
		if (scalar(keys %count) == 1) {
			return $towerref->{$root}{'weight'} - $imbalance
		} else {
			my ($bad, $good);
			foreach my $c (keys %count) {
				if ($count{$c} == 1) {
					$problem = $wt{$c};
					$bad = get_weight($towerref, $problem);
				} else {
					$good = get_weight($towerref, $wt{$c});
				}
			}
			$imbalance = $bad - $good;
			return balance_tower($towerref, $problem, $imbalance);
		}
	} else {
		return $towerref->{$root}{'weight'} - $imbalance;
	}
...
sub get_weight {
	...
	my $weight = $towerref->{$root}{'weight'};
	if (exists $towerref->{$root}{'child'}) {
		foreach my $c (@{$towerref->{$root}{'child'}}) {
			$weight += get_weight($towerref, $c);
		}
	}
	return $weight;
}
