...
my %reg = ();
my $largest_ever = parse_instructions(\%reg, \@lines);
my $largest = max(values %reg);
print "P1: The largest value in any register is $largest.\n";
print "P2: The largest value that was ever in any register is $largest_ever.\n";
...
sub parse_instructions {
	...
	foreach my $line (@$inref) {
		my @token = split(' ', $line);
		# format is 0:reg_to_change 1:inc/dec 2:amount 3:if 4:reg_to_check 5:inequality 6:check_amount
		$reg->{$token[0]} = 0 if (not exists $reg->{$token[0]});
		$reg->{$token[4]} = 0 if (not exists $reg->{$token[4]});
		if (eval "$reg->{$token[4]} $token[5] $token[6]") {
			$reg->{$token[0]} += ( $token[1] eq 'inc' ) ? $token[2] : -$token[2];
			$max = $reg->{$token[0]} if ($reg->{$token[0]} > $max);
		}
	}
	return $max;
}
