...
# For part 1, rather than running the simulation, we will compare absolute accelerations.
# Ties are then broken by absolute velocities and same for position. This will fail in certain situations
# (e.g. a = <1, 0, 0> for both and v1 = <-1, 0, 0> but v2 = <1, 0, 0> but p2 has shorter initial distance)
# but this is good enough for our puzzle inputs.
my %particles;
init_particles(\%particles, $puzzle);
my $closest = -1;
my $min_acc = 1e9;
my $min_vel = 1e9;
my $min_pos = 1e9;
foreach my $p (keys %particles) {
	my $abs_acc = get_distance($particles{$p}, 'a');
	my $abs_vel = get_distance($particles{$p}, 'v');
	my $abs_pos = get_distance($particles{$p}, 'p');
	
	my $comparison = ( $abs_acc <=> $min_acc or $abs_vel <=> $min_vel or $abs_pos <=> $min_pos );
	if ($comparison == -1) {
		$min_acc = $abs_acc;
		$min_vel = $abs_vel;
		$min_pos = $abs_pos;
		...
		warn "closest particle determination ended in a tie\n";
	}
}
print "P1: Long-term, the closest particle will be index $closest.\n";

# For part 2, we are actually running the sim. We need a stopping condition to determine that no more collisions
# will occur. This time we choose an arbitrarily large distance of 2500 units and assume that once all particles
# are at least that distance from the origin it is unlikely to see any more collisions.
init_particles(\%particles, $puzzle);
my $limit = 2500;
my $min_dist = 0;
while ($min_dist < $limit) {
	my %seen = ();
	$min_dist = $limit;
	foreach my $p (keys %particles) {
		...
		my $key = "$particles{$p}{'p'}{'x'},$particles{$p}{'p'}{'y'},$particles{$p}{'p'}{'z'}";
		$seen{$key} = [] unless (exists $seen{$key});
		push @{$seen{$key}}, $p;
		my $dist = get_distance($particles{$p});
		$min_dist = $dist if ($dist < $min_dist);
	}
	foreach my $key (keys %seen) {
		if (scalar(@{$seen{$key}}) > 1) {
			foreach my $p (@{$seen{$key}}) {
				delete $particles{$p};
			}
		}
	}
...
my $count = scalar(keys %particles);
print "P2: There will be $count particles left after collisions.\n";

# This is confusingly named since it will do the calculation on acceleration or velocity as well as position.
# It is also always compared to 0,0,0 rather than another particle.
sub get_distance {
	my $particle = shift;
	my $var = shift;
	$var = 'p' unless (defined $var);
	
	return abs($particle->{$var}{'x'}) + abs($particle->{$var}{'y'}) + abs($particle->{$var}{'z'});
}
...
sub init_particles {
	...
	foreach my $line (split("\n", $input)) {
		$pr->{$p} = { 'd' => 1e9 };
		foreach my $var (split(' ', $line)) {
			(my ($v, $x, $y, $z)) = $var =~ /^(\w)=<(-?\d+),(-?\d+),(-?\d+)>/;
			$pr->{$p}{$v} = {'x' => $x, 'y' => $y, 'z' => $z};
		}
		$p++;
	}
