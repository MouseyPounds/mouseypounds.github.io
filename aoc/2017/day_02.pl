...
my $checksum = calc_checksum(\@rows);
print "P1: The checksum is $checksum.\n";

$checksum = calc_divisors(\@rows);
print "P2: The checksum for evenly divisible values is $checksum.\n";
...
sub calc_divisors {
	...
	ROW: foreach my $r (@$rowref) {
		my @digits = sort {$a <=> $b} split(' ', $r);
		for (my $d = 0; $d <= $#digits; $d++) {
			for (my $n = $d + 1; $n <= $#digits; $n++) {
				if ($digits[$n] % $digits[$d] == 0) {
					$sum += $digits[$n] / $digits[$d];
					next ROW;
				}
			}
		}
