...
my $aunt = -1;
foreach my $a (keys %aunts) {
	my $found_her = 1;
	foreach my $p (keys %evidence) {
		if (exists $aunts{$a}{$p}) {
			$found_her = 0 unless ($aunts{$a}{$p} == $evidence{$p});
		}
	}
	if ($found_her) {
		$aunt = $a;
...
print "P1: Found the aunt who sent the gift: Sue $aunt\n";

my %greater = ( 'cats' => 1, 'trees' => 1 );
my %lesser = ( 'pomeranians' => 1, 'goldfish' => 1 );
my $aunt = -1;
foreach my $a (keys %aunts) {
	my $found_her = 1;
	foreach my $p (keys %evidence) {
...
print "P2: Found the aunt who sent the gift: Sue $aunt\n";
