...
#
# Initially tried using an array of strings to save some space, but in part 2 the possibility of brightness values over 10
# complicated the problem enough that we abandoned that for the simpler 2d array and then retrofiot that to part 1 as well.
...
for (my $in = 0; $in <= $#instructions; $in++) {
	$_ = $instructions[$in];
	print "Running $_          \r";
	my ($command, $x1, $y1, $x2, $y2) = /^(\D+)(\d+),(\d+) through (\d+),(\d+)/;
	
	if ($command =~ /^turn (\w+)/) {
		my $new_state = ($1 eq 'on') ? 1 : 0;
		for (my $i = $y1; $i <= $y2; $i++) {
			for (my $j = $x1; $j <= $x2; $j++) {
...
# Note: since off is 0 and on is 1, the brightness function still works for this.
print "P1: After following instructions, there are " . count_brightness() . " lights on.\n";
...
for (my $in = 0; $in <= $#instructions; $in++) {
	...
	for (my $i = $y1; $i <= $y2; $i++) {
		for (my $j = $x1; $j <= $x2; $j++) {
			$lights[$i][$j] = max(0, $lights[$i][$j] + $change);
		}
	}
}

print "P2: After following instructions, the total brightness is " . count_brightness() . ".\n";
...
# The original part 1 string implementation had done stuff like this:
# my %toggle = ( '0' => '1', '1' => '0' );
#	my $width = $x2 - $x1 + 1;
#	if ($command =~ /^turn (\w+)/) {
#		my $replace = ($1 eq 'on') ? "1" : "0";
#		for (my $i = $y1; $i <= $y2; $i++) {
#			substr($lights[$i], $x1, $width) = "$replace" x $width;
#		}
#	} elsif ($command =~ /^toggle/) {
#		for (my $i = $y1; $i <= $y2; $i++) {
#			substr($lights[$i], $x1, $width) =~ s/([01])/$toggle{$1}/ge;
#		}
#	}
#
#sub count_lights {
