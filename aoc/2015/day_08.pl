...
#
# Rather than trying to count the theoretical increases & decreases, actually changing the strings and then grabbing the length.
# Originally, we just substituted placeholders to get the counts quickly, but now we are doing proper substitutions.
...
while (<DATA>) {
	...
	s/^"//;
	s/"$//;
	# Hex code substitutions are a pain because we don't want them to mistakenly trigger on an escaped backslash.
	# We match two situations: A single backslash before the x or a larger odd number of backslashes before the x.
	# Using two regexes although a variable-length lookbehind assertion might be able to do it in one.
	s/(?<!\\)(\\\\)+\\(x[0-9a-fA-F]{2})/$1 . chr(hex($2))/ge;
	s/(?<!\\)\\(x[0-9a-fA-F]{2})/chr(hex($1))/ge;
	# Now that the hard stuff is done, just replace escaped backslashes and escaped quotes.
	s/\\\\/\\/g;
	s/\\"/"/g;
	$mem += length $_;
	print "{$_} OR " if $debugging;
	
	# Reset string, encode all quotes & backslashes, then add wrapping quotes.
	$_ = $original;
	s/\\/\\\\/g;
	s/"/\\"/g;
	$_ = qq("$_");
...
print "2015 Day 8\n";
print "P1: Text space was $text and memory space is $mem, a difference of " . ($text - $mem) . "\n";
print "P2: Encoded text space is $encoded and original text space was $text, a difference of " . ($encoded - $text) . "\n";
