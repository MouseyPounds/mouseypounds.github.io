...
#
# For part 2 we simplified the Rule 8 replacement to an equivalent `42+` and then used perl's recursive regex for Rule 11.
# The end result is a pattern ~7k characters long which is way better than the 64k we originally had when hardcoding
# 6 levels of the recursion into the pattern.
...
my @messages = grep { /^[a-z]/ } split("\n", $puzzle);
my @rule_queue = grep { /^\d+/ } split("\n", $puzzle);

my $rules = parse_rules(\@rule_queue);
my $count = grep { /^$rules->{'0'}$/ } @messages;
print "P1: Rule 0 matched $count messages. Its regex is " . (length $rules->{'0'}) . " characters long.\n";

@rule_queue = grep { /^\d+/ } split("\n", $puzzle);
$rules = parse_rules(\@rule_queue, 1);
$count = grep { /^$rules->{'0'}$/ } @messages;
print "P2: Rule 0 matched $count messages after rule changes. Its regex is " . (length $rules->{'0'}) . " characters long.\n";

sub parse_rules {
	my $queue = shift;
	my $replace_rules = shift;
	
	$replace_rules = 0 unless (defined $replace_rules);
	my %rules = ();

	while (my $r = shift @$queue) {
		if ($replace_rules) {
			$r = "8: 42+" if ($r =~ /^8:/);
			$r = "11: (?<P>42 31 | 42 (?&P) 31)" if ($r =~ /^11:/);
		}
		(my ($id, $matchstr)) = $r =~ /^(\d+): (\S.*)/;
		if ($matchstr =~ /^"(\w)"/) {
			$rules{$id} = $1;
		} else {
			my $have_all = 1;
			foreach my $n ($matchstr =~ /(\d+)/g) { $have_all = 0 unless exists $rules{$n}; }
			if ($have_all) {
				$matchstr =~ s/(\d+)/($rules{$1})/g;
				$matchstr =~ s/ //g;
				$rules{$id} = $matchstr;
			} else {
				push @$queue, $r;
