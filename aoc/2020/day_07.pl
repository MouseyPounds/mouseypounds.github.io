...
# $bags{outer color}{inner color} = inner color qty
# $parents{inner color} = outer color
my %bags = ();
my %parents = ();
...
while (<DATA>) {
	chomp;
	my ($outer, $inner) = /^(.*) bags contain (.*)\.$/;
	my @details = split(', ', $inner);
	$bags{$outer} = {};
	foreach my $b (@details) {
		next if ($b =~ /no other bags/);
		my ($qty, $color) = $b =~ /^(\d+) (.*) bags?$/;
		$bags{$outer}{$color} = $qty;
		$parents{$color} = [] if (not exists $parents{$color});
		push @{$parents{$color}}, $outer;
	}
}

my $target = "shiny gold";
my %results = ();
get_parents($target, \%results);
print "P1 There are " . scalar(keys %results) . " bag types which can contain $target bags.\n";

my $contains = 0;
my $count = count_contents($target);
print "P2 There are $count bags inside $target bags.\n";

# recursively count how many bags inside given bag. the recursive part will count 1 too many because it
# includes the outer bag. using a wrapper to counter that.
sub count_contents {
	return count_bags(shift) - 1;
}
...
sub count_bags {
	...
	if (exists $bags{$t}) {
		foreach my $b (keys %{$bags{$t}}) {
			$r += $bags{$t}{$b} * count_bags($b);
		}
	}
	return $r;
}

# recursively gather possible parents of given bag
sub get_parents {
	my $t = shift;
	my $r = shift;
	my $count = 1;
