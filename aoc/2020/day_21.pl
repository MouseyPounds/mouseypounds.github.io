...
my %allergens = ();
my @rules = ();
parse_input(\%ingredients, \%allergens, \@rules, \@lines);

reduce_ingredients(\%ingredients, \%allergens, \@rules);

my $total = 0;
map { $total += $ingredients{$_}{'COUNT'} if (scalar keys %{$ingredients{$_}} == 1) } keys %ingredients;
print "P1: Safe ingredients appear $total times.\n\n";

print "Allergen : Ingredient mapping\n";
my $list = "";
foreach my $a (sort keys %allergens) {
	my $i = join('|', sort keys %{$allergens{$a}});
	$list .= "$i,";
	print "  $a : $i\n";
}
chop $list;
print "P2: Dangerous ingredient list: $list\n";

sub reduce_ingredients {
	my $ing_ref = shift;
	my $all_ref = shift;
	my $rule_ref = shift;

	# Initialize each ingr with every possible allergen and the reverse
	foreach my $i (keys %$ing_ref) {
		foreach my $a (keys %$all_ref) {
			$ing_ref->{$i}{$a} = "";
			$all_ref->{$a}{$i} = "";
		}
	}
	
	# Anytime an ingredient is not listed in a rule, it cannot have any of the allergens in that rule
	foreach my $r (@$rule_ref) {
		foreach my $i (keys %$ing_ref) {
			if (not exists $r->{'i'}{$i}) {
				foreach my $a (keys %{$r->{'a'}}) {
					delete $ing_ref->{$i}{$a} if (exists $ing_ref->{$i}{$a});
					delete $all_ref->{$a}{$i} if (exists $all_ref->{$a}{$i});
				}
			}
		}
	}
	
	# Stopping here is actually enough for part 1, but we don't yet have all we need for
	# part 2, so we try to reduce further via successively processing guaranteed results.
	my $check_again = 1;
	my %to_check = ();
	map { $to_check{$_} = ""; } keys %$all_ref;
	while ($check_again) {
		$check_again = 0;
		my %singles = ();
		foreach my $i (keys %$ing_ref) {
			if (scalar (keys %{$ing_ref->{$i}}) == 2) {
				my $allergen;
				foreach my $a (keys %{$ing_ref->{$i}}) { $allergen = $a if ($a ne 'COUNT'); }
				if (exists $to_check{$allergen}) {
					$singles{$allergen} = $i;
					delete $to_check{$allergen};
					$check_again = 1;
				}
			}
		}
		foreach my $s (keys %singles) {
			foreach my $i (keys %$ing_ref) {
				next if ($i eq $singles{$s});
				delete $ing_ref->{$i}{$s} if (exists $ing_ref->{$i}{$s});
			}
			$all_ref->{$s} = { $singles{$s} => "" };
			foreach my $a (keys %$all_ref) {
				next if ($a eq $s);
				delete $all_ref->{$a}{$singles{$s}} if (exists $all_ref->{$a}{$singles{$s}});
			}
		}
	}
}
...
sub parse_input {
	...
	foreach my $line (@$lines) {
		(my ($ings, $allers)) = $line =~ /^([^\(]+) \(contains ([^\)]+)\)/;
		my $rule = { 'i' => {}, 'a' => {} };
		foreach my $i (split(' ', $ings)) {
			$ing_ref->{$i}{'COUNT'} = 0 unless (exists $ing_ref->{$i}{'COUNT'});
			$ing_ref->{$i}{'COUNT'}++;
			$rule->{'i'}{$i} = "";
			foreach my $a (split(', ', $allers)) {
				$all_ref->{$a} = {};
				$rule->{'a'}{$a} = "";
			}
		}
