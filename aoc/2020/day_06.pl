...
while (<DATA>) {
	...
	if ($_ eq "") {
		...
		foreach my $k (keys %group) {
			$every_sum++ if ($group{$k} == $group_members);
		}
		%group = ();
		$group_members = 0;
	} else {
		foreach my $i (split ('')) {
			$group{$i} = 0 if (not exists $group{$i});
			$group{$i}++;
		}
		$group_members++;
	}
}
# Still need to count last group since no blank line at end of input
$sum += scalar(keys %group);
foreach my $k (keys %group) {
	$every_sum++ if ($group{$k} == $group_members);
}

print "P1: Total of all counts is $sum\n";
print "P2: Total of all counts is $every_sum\n";
