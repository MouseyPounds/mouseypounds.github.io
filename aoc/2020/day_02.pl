...
while (<DATA>) {
	chomp;
	my ($min, $max, $letter, $pass) = /(\d+)-(\d+) (\w): (\w+)/;
	
	my @matches = $pass =~ /$letter/g;
	$valid++ if (scalar(@matches) >= $min and scalar(@matches) <= $max);
	
	my $first = substr $pass, $min-1, 1;
	my $second = substr $pass, $max-1, 1;
	$valid_p2++ if (($first eq $letter and $second ne $letter) or ($first ne $letter and $second eq $letter));
}

print "p1: $valid valid\np2: $valid_p2 valid\n";
