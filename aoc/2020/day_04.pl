...
my %valid_eye_colors = ('amb' => 1, 'blu' => 1, 'brn' => 1, 'gry' => 1, 'grn' => 1, 'hzl' => 1, 'oth' => 1);
...
push @passports, $current if (scalar keys %$current > 0);
...
foreach my $p (@passports) {
	...
	$valid++ if ($isvalid);
	$validp2++ if ($isvalid and $isvalidp2);
}
print "P1: Found $valid valid passports out of $count total\n";
print "P2: Found $validp2 valid passports out of $count total\n";
...
sub check {
	...
	if ($field eq 'byr') {
		my ($yr) = $data =~ /^(\d{4})$/;
		$ok = ($yr ne "" and $yr >= 1920 and $yr <= 2002);
	} elsif ($field eq 'iyr') {
		my ($yr) = $data =~ /^(\d{4})$/;
		$ok = ($yr ne "" and $yr >= 2010 and $yr <= 2020);
	} elsif ($field eq 'eyr') {
		my ($yr) = $data =~ /^(\d{4})$/;
		$ok = ($yr ne "" and $yr >= 2020 and $yr <= 2030);
	} elsif ($field eq 'hgt') {
		my ($num, $unit) = $data =~ /^(\d+)(\w+)$/;
		if ($num ne '' and $unit ne '') {
			$ok = (($unit eq 'cm' and $num >= 150 and $num <= 193) or ($unit eq 'in' and $num >= 59 and $num <= 76));
		} else {
			$ok = 0;
		}
	} elsif ($field eq 'hcl') {
		my ($hc) = $data =~ /^\#([a-f0-9]{6})$/;
		$ok = (defined $hc and $hc ne '');
	} elsif ($field eq 'ecl') {
		my ($ec) = $data =~ /^(\w{3})$/;
		$ok = (defined $ec and exists $valid_eye_colors{$ec});
	} elsif ($field eq 'pid') {
		my ($id) = $data =~ /^(\d{9})$/;
		$ok = (defined $id and $id ne '');
	} elsif ($field eq 'cid') {
		$ok = 1;
	} else {
		$ok = 0;
	}
	#print "Checking {$field} {$data}: $ok\n";
	return $ok;
}
