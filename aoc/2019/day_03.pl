	...
	foreach my $m (@moves) {
		...
		if ($dir eq 'R') {
			for (my $i = 1; $i <= $len; $i++) {
				$x++;
				$steps++;
				if (not exists $points[$line]{"$x,$y"}) {
					$points[$line]{"$x,$y"} = $steps;
				}
			}
		} elsif ($dir eq 'L') {
			for (my $i = 1; $i <= $len; $i++) {
...
foreach my $p (keys %{$points[0]}) {
	if (exists $points[1]{$p}) {
		my @coords = split /,/, $p;
		my $d = abs($coords[0]) + abs($coords[1]);
		my $s = $points[0]{$p} + $points[1]{$p};
		print "Intersection at $p with distance $d and steps $s";
		if ($d < $min_dist) {
			print " NEW MIN DIST";
			$min_dist = $d;
		}
...
print "\nMin Dist was $min_dist\nMin Steps was $min_steps";
