#!/bin/perl -w
#
# https://adventofcode.com/2021/day/18
#

use strict;
use POSIX qw(floor ceil);

print "2021 Day 18\n";
my $input = do { local $/; <DATA> }; # slurp it
my @numbers = map { [ m/[^,]/g ] } split("\n", $input);

my $sum = $numbers[0];
for (my $i = 1; $i <= $#numbers; $i++) {
	$sum = add_snailfish($sum,$numbers[$i]);
}
print "Part 1: Magnitude of sum is ", get_magnitude($sum), "\n";

my $max_sum = 0;
for (my $i = 0; $i <= $#numbers; $i++) {
	for (my $j = 0; $j <= $#numbers; $j++) {
		next if ($i == $j);
		$sum = get_magnitude(add_snailfish($numbers[$i], $numbers[$j]));
		$max_sum = $sum if ($sum > $max_sum);
	}
}
print "Part 2: Largest magnitude of sum of 2 numbers is $max_sum\n";

sub sprint_snailfish {
	my $a = shift;
	return join(" ", @$a);
}

sub get_magnitude {
	my $a = shift;
	my @copy = @$a;
	while (scalar(@copy) > 1) {
		for (my $i = 0; $i <= $#copy; $i++) {
			if (num_pair_at(\@copy, $i)) {
				splice(@copy, $i, 4, 3*$copy[$i+1] + 2*$copy[$i+2]);
				last;
			}
		}
	}
	return $copy[0];
}

sub add_snailfish {
	my $a = shift;
	my $b = shift;

	my $c = [ '[', @$a, @$b, ']' ];
	reduce_snailfish($c);
	return $c;
}

sub reduce_snailfish {
	my $a = shift;

	my $check_again = 1;
	LOOP: while ($check_again) {
		$check_again = 0;
		# Check for Explodes
		my $depth = 0;
		my $pos;
		for (my $i = 0; $i <= $#$a; $i++) {
			if ($depth >= 4 and num_pair_at($a, $i)) {
				$pos = $i;
				last;
			} elsif (is_open($a->[$i])) {
				$depth++;
			} elsif (is_close($a->[$i])) {
				$depth--;
			}
		}
		if (defined $pos) {
			for (my $j = $pos; $j >= 0; $j--) {
				if (is_num($a->[$j])) {
					$a->[$j] += $a->[$pos+1];
					last;
				}
			}
			for (my $j = $pos+4; $j <= $#$a; $j++) {
				if (is_num($a->[$j])) {
					$a->[$j] += $a->[$pos+2];
					last;
				}
			}
			splice(@$a, $pos, 4, 0);
			$check_again = 1;
			next LOOP;
		}
		# Check for Splits
		for (my $i = 0; $i <= $#$a; $i++) {
			if (is_num($a->[$i]) and $a->[$i] > 9) {
				splice(@$a, $i, 1, '[', floor($a->[$i]/2), ceil($a->[$i]/2), ']');
				$check_again = 1;
				next LOOP;
			}
		}
	}
}

sub is_open {
	my $c = shift;
	return $c eq '[';
}

sub is_close {
	my $c = shift;
	return $c eq ']';
}

sub is_num {
	my $c = shift;
	return $c =~ /\d/;
}

sub num_pair_at {
	my $a = shift;
	my $i = shift;

	return (is_open($a->[$i]) and is_num($a->[$i+1]) and is_num($a->[$i+2]));
}

__DATA__
[[[6,[8,3]],[2,0]],[[[9,5],[9,1]],3]]
[[[9,[2,2]],[5,4]],[[[2,2],[9,6]],[7,7]]]
[[[0,[3,2]],1],[[0,[2,8]],[2,[0,4]]]]
[[4,4],[[[7,0],5],[3,1]]]
[[5,4],1]
[[[[7,6],4],9],[[9,1],9]]
[[[1,[7,8]],[[7,7],[1,6]]],[1,[6,[7,1]]]]
[[[[6,8],[5,6]],[[1,1],8]],[[[2,0],[3,1]],[2,[2,6]]]]
[[[6,3],[3,[7,1]]],8]
[[[9,4],[3,[0,6]]],[[2,[3,6]],[[9,8],[1,6]]]]
[9,[0,[[0,7],2]]]
[[[[8,4],7],[[9,2],[0,9]]],[[7,9],[8,[0,9]]]]
[[1,1],[[5,[3,8]],[3,[4,7]]]]
[[[9,[2,9]],[2,[2,9]]],[[[3,5],5],[[3,3],2]]]
[[[[5,4],9],0],[[[5,7],2],[[5,2],9]]]
[[2,[[1,0],[6,2]]],0]
[[[3,7],[7,6]],[[[2,8],5],[3,[9,7]]]]
[[2,[2,[8,8]]],[[[9,9],[1,1]],[[8,6],[0,3]]]]
[[8,1],[3,5]]
[[7,[[7,6],[2,0]]],4]
[[5,4],[[1,3],[5,[2,8]]]]
[7,9]
[[[[6,9],0],[1,[5,0]]],[[[6,4],3],7]]
[[[[3,7],3],[2,6]],[[0,4],[9,9]]]
[[[[1,5],[5,0]],[9,4]],[[[8,3],3],[8,[3,6]]]]
[[[[3,7],5],[[8,5],[1,5]]],[[0,6],[3,4]]]
[[[[4,0],2],[7,[8,4]]],[0,[5,[7,8]]]]
[[[[0,8],[0,4]],[9,3]],[[[5,4],[4,8]],[[1,6],[5,4]]]]
[[0,[0,3]],[[3,[1,5]],[[9,6],[0,6]]]]
[[9,[8,4]],[7,1]]
[[[[1,9],[7,7]],9],[[6,[4,5]],[8,[3,2]]]]
[5,[[2,[9,5]],[3,[4,0]]]]
[[[6,2],[[1,8],5]],6]
[[8,[6,[6,4]]],[0,[[9,8],7]]]
[[[[6,3],[8,0]],[8,[2,7]]],8]
[[[6,[3,6]],[[4,0],[4,7]]],[0,[[4,0],[4,5]]]]
[[[3,[8,1]],1],[2,3]]
[[[6,[7,0]],[[3,5],[3,4]]],7]
[[[[8,0],3],8],[[[1,6],3],[[0,5],2]]]
[[[3,7],[[9,8],8]],[[[8,4],7],[3,[1,7]]]]
[[[0,5],[[5,5],[7,8]]],[9,[5,[2,2]]]]
[[2,9],[[[7,4],4],[[8,0],[6,9]]]]
[[[[7,8],[8,8]],0],9]
[[[4,[0,6]],[[5,9],[0,1]]],[3,[6,7]]]
[[[7,[6,9]],[5,[6,4]]],[[[3,9],6],[[0,1],1]]]
[3,[[[6,9],7],[5,8]]]
[[[3,9],[[3,5],2]],[[[2,5],[4,6]],[8,0]]]
[[[9,7],3],[[[2,7],[0,9]],[3,[0,3]]]]
[[3,[4,0]],[[6,6],[4,5]]]
[[0,0],[[5,9],1]]
[[[6,8],[2,6]],[[[1,1],3],7]]
[[[4,4],[[1,0],[2,4]]],[2,6]]
[[[[6,0],6],[8,[9,9]]],[[4,2],[[1,8],[5,3]]]]
[[[[1,6],[4,3]],[5,5]],[[7,[9,9]],4]]
[[[[6,9],7],[9,3]],[[[9,6],5],0]]
[[3,[[7,2],[8,1]]],[[7,[3,0]],1]]
[0,[0,[1,3]]]
[[[0,5],[[6,1],[4,6]]],[[[0,4],8],[[4,5],9]]]
[[[[7,5],[7,0]],[6,[7,2]]],[7,[3,[4,1]]]]
[[3,3],[0,[6,2]]]
[[[3,8],[[7,3],6]],[[[0,8],3],[[8,9],[2,9]]]]
[[4,[[5,6],[4,0]]],[[7,[7,5]],[5,0]]]
[[[[2,5],[5,4]],9],[[[6,0],[0,0]],[[5,1],8]]]
[[2,[[1,7],7]],[[[4,5],[7,9]],0]]
[[[0,9],[[5,4],3]],3]
[[9,[[1,9],[1,6]]],[[9,[0,3]],[[8,8],[0,7]]]]
[[[[7,2],4],[7,8]],[[[4,1],[3,1]],[2,5]]]
[[[[1,8],3],[2,5]],[[0,[5,8]],[[1,3],[5,2]]]]
[[3,9],[[9,6],[5,[7,1]]]]
[1,[[3,[6,5]],[5,[2,7]]]]
[[[5,8],6],[8,[[9,4],[0,4]]]]
[0,[[5,[6,6]],[[7,4],[4,6]]]]
[[[[6,8],2],[[1,6],[8,2]]],6]
[7,2]
[[3,1],7]
[[[2,[9,5]],0],[[[7,3],4],8]]
[[[[0,0],[4,2]],5],[[8,6],2]]
[[1,[7,8]],[2,[[6,6],[5,7]]]]
[[[3,[6,0]],3],[[7,[4,4]],8]]
[[[9,[8,7]],[[4,2],4]],[[6,1],[[3,3],[2,2]]]]
[[[8,1],[[7,4],[5,9]]],9]
[[[2,[8,6]],[[9,8],2]],[[9,5],[1,[9,8]]]]
[[[[6,1],[3,1]],[[4,5],1]],[[[6,4],[6,2]],2]]
[[[[4,0],[0,1]],[[1,4],6]],7]
[[[[8,9],[0,2]],4],[[[9,8],8],[0,[0,6]]]]
[0,[[[0,9],1],7]]
[[1,[[3,7],3]],[[[2,4],3],0]]
[[[[7,6],3],8],[[5,5],9]]
[[2,[1,3]],[[[6,7],3],[[3,8],7]]]
[[[[0,6],6],6],[[5,[0,9]],[8,[2,4]]]]
[4,[[[3,0],[2,5]],[[7,4],1]]]
[[[[7,9],3],[0,[8,2]]],[[8,[3,4]],[[2,3],[1,6]]]]
[[[3,[6,3]],5],[[3,4],2]]
[[[[1,9],[0,3]],[0,8]],[[[4,2],[4,3]],[[8,9],5]]]
[[[[2,8],[4,9]],[[3,5],6]],[[6,[1,5]],[0,[9,7]]]]
[[6,3],[[[7,7],[1,7]],[[6,5],[0,8]]]]
[[1,[1,[5,8]]],7]
[[0,6],[9,[[3,4],0]]]
[[[[0,2],7],9],9]
[9,6]