...
foreach my $i (split("\n", $input)) {
	my @patterns = $i =~ /(\w+)/g;
	my @digits = splice(@patterns, 10, 4);
	my @segments = ('.') x 7;
	my @keys = ("") x 7;
	my %translate = ();
	my %letter_counts = ();
	my %leftovers = ('5' => [], '6' => []);
	# sort the patterns and grab the 4 that have unique lengths
	for (my $p = 0; $p <= $#patterns; $p++) {
		$patterns[$p] = join('', sort split('', $patterns[$p]));
		my $len = length $patterns[$p];
		if ($len == 2) {
			$translate{$patterns[$p]} = 1; $keys[1] = $patterns[$p];
		} elsif ($len == 3) {
			$translate{$patterns[$p]} = 7; $keys[7] = $patterns[$p];
		} elsif ($len == 4) {
			$translate{$patterns[$p]} = 4; $keys[4] = $patterns[$p];
		} elsif ($len == 7) {
			$translate{$patterns[$p]} = 8; $keys[8] = $patterns[$p];
		} else {
			push @{$leftovers{$len}}, $patterns[$p];
		}
		# Count the letters which we will use later
		foreach my $letter (split('', $patterns[$p])) {
			$letter_counts{$letter}++;
		}
	}
	# Assign segments based on unique counts
	foreach my $letter (keys %letter_counts) {
		if ($letter_counts{$letter} == 4) {
			$segments[4] = $letter;
		} elsif ($letter_counts{$letter} == 6) {
			$segments[1] = $letter;
		} elsif ($letter_counts{$letter} == 9) {
			$segments[5] = $letter;
		}
	}
	# Whatever letter is in digit 7 but not digit 1 is segment 0
	foreach my $letter (split('', $keys[7])) {
		if (index($keys[1], "$letter") == -1) {
			$segments[0] = $letter; last;
		}
	}
	# Digit 1 is made from segments 2 & 5 and we already know segment 5
	foreach my $letter (split('', $keys[1])) {
		if ($letter ne $segments[5]) {
			$segments[2] = $letter; last;
		}
	}
	# Digit 4 has segments 1,2,3, & 5 and we know all of these except 3
	foreach my $letter (split('', $keys[4])) {
		if ($letter ne $segments[1] and $letter ne $segments[2] and $letter ne $segments[5]) {
			$segments[3] = $letter; last;
		}
	}
	# Segments 3 and 6 are both in 7 patterns and we know segment 3
	foreach my $letter ('a' .. 'g') {
		if ($letter_counts{$letter} == 7 and $segments[3] ne $letter) {
			$segments[6] = $letter; last;
		}
	}
	# We now know which letters light up which segments. All that is left is to finish translating
	foreach my $p (@{$leftovers{6}}) {
		if (index($p, "$segments[2]") == -1) {
			$translate{$p} = 6; $keys[6] = $p;
		} elsif (index($p, "$segments[3]") == -1) {
			$translate{$p} = 0; $keys[0] = $p;
		} else {
			$translate{$p} = 9; $keys[9] = $p;
		}
	}
	foreach my $p (@{$leftovers{5}}) {
		if (index($p, "$segments[4]") > -1) {
			$translate{$p} = 2; $keys[2] = $p;
		} elsif (index($p, "$segments[1]") > -1) {
			$translate{$p} = 5; $keys[5] = $p;
		} else {
			$translate{$p} = 3; $keys[3] = $p;
		}
	}
	my $output = "";
	foreach my $d (@digits) {
		my $len = length $d;
		$p1_count++ if ($len < 5 or $len > 6);
		my $key = join('', sort split('', $d));
		die "Unknown digit {$d} -> {$key}" unless (exists $translate{$key});
...
print "P1: There are $p1_count unique digits in the output.\n";
print "P2: The sum of the outputs is $p2_count.\n";
